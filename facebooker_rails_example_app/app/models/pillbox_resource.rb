    ...
    class Base           
      	    def self.instantiate_collection(collection, prefix_options = {})
                if collection.is_a?(Hash) && collection.size == 1
                  case value = collection.values.first
                  when Array
...
#    rescue NoMethodError => e # means empty ... because of the duckpunching I did to AResource above... # method 'name' on nil
#      [] if e.to_s.include? 'name'
#    end  
  end
  def self.interpret_params(options = {})
    ...
    if params[:start]
       params['lower_limit'] = (params[:page] || "1").to_i * params[:start].to_i
    end
    params.delete(:page)
    params.delete(:start)
    ...
    begin
      ...
      when NilClass; 
      when Array;                          color_code(params['color']).join(";")
    ...
    begin
      ...
      when NilClass; 
      when Array;                         shape_code(params['shape']).join(";")  
  ...
  def shape_code(*shape_name_or_code)
    SHAPES[shape_name_or_code.to_s.upcase] || shape_name_or_code
  end
  def color_code(*color_name_or_code)
    COLORS[color_name_or_code.to_s.upcase] || color_name_or_code
  end
  
  VALID_ATTRIBUTE_NAMES = %w(key color color2 ingredient shape imprint prodcode has_image size lower_limit)
  def searchable_attribute
    VALID_ATTRIBUTE_NAMES.inject({}) {|i,k| 
         if self.respond_to?(k)
           v = self.send k
           i.merge!(k => v) unless v.blank?
         end
         i
         }
  end
  ...
  def create_pill(options = {})
    begin
      Pill.create do |new_pill|
        options = options.inject({}) {|i,k| i.merge(k=>nil) } if options.is_a? Array
         for method, default in options
           default = nil if default.is_a? TrueClass
           new_pill.write_attribute method, (default.nil? ? self.send(method) : default)
         end
         
      end
    rescue => e
      puts "can't build #{ ingredient || trade_name }"
    end      
  end
...
after you have loaded some PillboxResource object into lvar "pr", then execute:
pr.create_pill(:name=>pr.ingredient,:accepted_names=>[pr.ingredient, pr.trade_name], :prodcode=>true, :image_ref=>pr.image_url, :api_ref=>pr.api_url, :drug_class=>1,:pill_category_id=>2)
